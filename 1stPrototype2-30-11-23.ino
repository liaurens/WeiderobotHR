...
bool approx(double current,  double goal, double margin){
  return current <= goal + margin && current >= goal - margin;
}
...
double calcBearing(double curLat, double curLon,double desLat,double desLon){
  double bearing = atan2((sin((desLon - curLon)*M_PI/180) * cos(desLat*M_PI/180)),
                         (cos(curLat*M_PI/180)*sin(desLat*M_PI/180)) - (sin(curLat*M_PI/180)*cos(desLat*M_PI/180)*cos((desLon-curLon)*M_PI/180)));
  if(bearing < 0){
    bearing += 2*PI;
  }
  if(bearing > 2*PI){
...
void loop() {
  ...
  // Else part for when there is no GPS fix
  else {
    // Actions to take when GPS doesn't have a fix
    // For example, stop all movement or maintain current state
  }
}
